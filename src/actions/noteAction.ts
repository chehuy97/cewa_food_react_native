...
import {
  ...
  edit_note,
  remove_note,
  add_new_user_in_note,
} from '../service/network';

export const fetch_all_notes = (
  account_id: string,
  token: string,
): ThunkAction<Promise<void>, {}, {}, noteAction> => {
  return (dispatch: ThunkDispatch<{}, {}, noteAction>) => {
    return get_all_notes(account_id, token)
      .then(res => {
...
export const add_note = (
  ...
  return (dispatch: ThunkDispatch<{}, {}, noteAction>) => {
    return add_new_note(note, token)
      .then(res => {
...
export const edit_one_note = (
  note: INote,
  token: string,
): ThunkAction<Promise<void>, {}, {}, noteAction> => {
  return (dispatch: ThunkDispatch<{}, {}, noteAction>) => {
    return edit_note(note, token)
      .then(res => {
...
export const remove_one_note = (
  note_id: string,
  token: string,
): ThunkAction<Promise<void>, {}, {}, noteAction> => {
  return (dispatch: ThunkDispatch<{}, {}, noteAction>) => {
    return remove_note(note_id, token)
      .then(res => {
...
export const add_new_email_in_note = (
  email: string,
  note_id: string,
  token: string,
  ...
  return (dispatch: ThunkDispatch<{}, {}, noteAction>) => {
    return add_new_user_in_note(email, note_id, token)
      .then(res => {
      ...
      .catch(err => {
        dispatch(note_failure(err.response.data.errorMessage));
      });
  };
};
